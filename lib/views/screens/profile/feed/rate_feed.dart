...
class RateFeed extends StatefulWidget {
  ...
  const RateFeed({super.key, required this.feed});

  @override
  State<RateFeed> createState() => _RateOutfitState();
}

class _RateOutfitState extends State<RateFeed> {
  ...
  void onSubmit() async {
    final message = await feedCtrl.ratePost(
      widget.feed.id,
                  ...
                  Expanded(
                    child: Obx(
                      () => CustomButton(
                        text: "rate_now".tr,
                        onTap: onSubmit,
                        isLoading: feedCtrl.isRatingLoading.value,
                      ),
                    ),
                  ),
                  const SizedBox(width: 20),
